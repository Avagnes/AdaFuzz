...
class CureFuzz:
    def __init__(self, input_size=5, hidden_size=64, output_size=64,learning_rate=1e-4):
        self.corpus = []
        self.final_state = []
        self.rewards = []
        self.result = []
    ...
    def get_pose(self):
        ...
        for i in range(len(self.corpus)):
            prob = self.intrinsic_reward[i]+math.exp(round(-self.rewards[i])*0.1)+self.entropy[i]
        ...
        if self.count[choose_index] <= 0:
            ...
            self.current_index = None
        return self.current_pose, self.current_final_state
