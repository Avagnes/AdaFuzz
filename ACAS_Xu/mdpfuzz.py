...
class ACASagent:
    def __init__(self, acas_speed):
        ...
        self.model_1 = load_repair(algo, model_index=1)
        self.model_2 = read_onnx(2, 2)
        self.model_3 = read_onnx(3, 2)
        # self.model_4 = read_onnx(4, 2)
        self.model_4 = load_repair(algo, model_index=4)
        # self.model_5 = read_onnx(5, 2)
        self.model_5 = load_repair(algo, model_index=5)
        self.prev_action = 0
...
def fuzz_testing(fuzzer, args):
    ...
    while len(fuzzer.corpus) > 0:
        ...
        if collide_flag:
            ...
            result_time.append(current_fuzz_time - start_fuzz_time)
    ...
    with open("MDPFuzz"+datetime.now().strftime("%m_%d_%Y_%H_%M_%S"), "wb") as f:
        pickle.dump((all_seeds, fuzzer.result, result_time), f)
    print('finish')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--picklepath", type=str, default='results.pkl')
