...
result_folder = "ablation"+datetime.now().strftime("%m_%d_%Y_%H_%M_%S")
os.mkdir('./results'+result_folder+'/')
os.mkdir('./results'+result_folder+'/artifact')
...
def sample_seeds(seeds_num=1000):
    ...
    while len(fuzzer.corpus) < seeds_num:
        ...
        intrinsic_reward = fuzzer.train_rnd(states_seq)[0]
        break_flag = 0
        mutated_acas_speed, mutated_x2, mutated_y2, mutated_auto_theta = fuzzer.mutate(acas_speed, x2, y2, auto_theta)
        while verify(mutated_acas_speed, mutated_x2, mutated_y2, mutated_auto_theta) == False:
            break_flag += 1
            mutated_acas_speed, mutated_x2, mutated_x2, mutated_auto_theta = fuzzer.mutate(acas_speed, x2, y2, auto_theta)
            if break_flag > 50:
                break
        reward, _, mutated_states_seq = reward_func(acas_speed, x2, y2, auto_theta)
        entropy = np.linalg.norm(np.asarray(states_seq[-1]) - np.asarray(mutated_states_seq[-1]))
    
        fuzzer.further_mutation(current_pose, reward, entropy, intrinsic_reward, states_seq[-1],current_pose)
        pbar.update(1)
    
    for i in tqdm.tqdm(range(len(fuzzer.corpus))):
        in_reward = fuzzer.compute_intrinsic_reward(get_state(*fuzzer.corpus[i]))
        fuzzer.intrinsic_reward[i] = in_reward
...
def fuzz_testing(fuzzer, args):  
    intrinsic_threshold = args.intrinsic
    ...
    while len(fuzzer.corpus) > 0:
        temp1 = time.time()
        orig_pose, final_state = fuzzer.get_pose()
        ...
        if break_flag > 50:
            fuzzer.drop_current()
            continue
        all_seeds.append((acas_speed, x2, y2, auto_theta))
        reward, collide_flag, mutated_states_seq = reward_func(acas_speed, x2, y2, auto_theta)
        intrinsic_reward = fuzzer.train_rnd(mutated_states_seq)[0]
        entropy = np.linalg.norm(np.asarray(final_state) - np.asarray(mutated_states_seq[-1]))
        if collide_flag:
            fuzzer.add_crash((acas_speed, x2, y2, auto_theta))
            result_time.append(time.time())
            print('found: ', len(fuzzer.result))
        elif reward < fuzzer.current_reward or intrinsic_reward > intrinsic_threshold or entropy > entropy_threshold:
            current_pose = [acas_speed, x2, y2, auto_theta]
            orig_pose = fuzzer.current_original
            fuzzer.further_mutation(current_pose, reward, entropy, intrinsic_reward, mutated_states_seq[-1], current_pose)
    ...
    with open('./results'+result_folder+'/artifact/run_crash_timing.pkl', 'wb') as handle:
        pickle.dump((all_seeds, fuzzer.result, np.array(result_time)-start_fuzz_time), handle, protocol=pickle.HIGHEST_PROTOCOL)
    ...
    for i in range(len(result)):
        for m in range(3):
            if m == 0:
                ...
                gif_name = './results'+result_folder+'/artifact/crash/' + str(i) + '_' + str(min_row) + '.gif'
                ani.save(gif_name)
            else:
                pass
                X1_3 = []
                ...
                ani = animation.FuncAnimation(fig, animate, init_func=init,
                                    frames=len(X1), interval=1, blit=True)
                gif_name = './results'+result_folder+'/artifact/noncrash/' + str(i) + '_' + str(min_row) + '.gif'
                ani.save(gif_name)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--seed_size", type=float, default=10000)
    parser.add_argument("--picklepath", type=str, default='')
    parser.add_argument("--intrinsic", type=int, default=500)
    parser.add_argument("--entropy", type=int, default=500)
    ...
    if args.replay and args.repair:
        ...
        f = open('./results'+result_folder+'/artifact/fuzzing_cure_log.txt', 'w', buffering=1)
        sys.stdout = f
