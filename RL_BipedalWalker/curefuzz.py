...
def main():
    ...
    parser.add_argument("--guide", action="store_true", default=True)
    ...
    all_seeds = []
    result_time = []
    while current_time - start_fuzz_time < (3600 * 1) and len(fuzzer.corpus) > 1:
        states = fuzzer.get_pose()
        mutate_states = fuzzer.mutation(states)
        all_seeds.append(mutate_states)
        state = None
        episode_reward = 0.0
        obs = env.reset(mutate_states)
        sequences = [obs[0]]
