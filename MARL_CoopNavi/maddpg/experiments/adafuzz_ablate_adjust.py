...
def parse_args():
    ...
    parser.add_argument("--test-times", help="mutation times limit", default=0, type=int)
...
def ada_test(arglist):
        ...
        while time.time() - start_test_time < 3600 * arglist.hours:
            while len(tester.families) < arglist.family_size:
                ...
                while True:
                    ...
                    # print(action_n, action_n[0], type(action_n), type(action_n[0]))
            ...
            # steps, mutate_reward, sequence,win = mdp.test(mutate_seed)
            # print(mutate_seed)
            win = 0
            mdp_test_time += 1
            # if pbar is not None:
    ...
    with open('ablate_adjust', 'wb') as handle:
