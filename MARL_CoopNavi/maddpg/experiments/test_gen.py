...
def parse_args():
    ...
    parser.add_argument("--hour", help="test time", default=1.0, type=float)
...
def test(arglist):
    ...
    with U.single_threaded_session():
        ...
        failure_count_list = [[0, 0]]
        failure_cluster_count_list = [[0, 0]]
        all_seeds = []
                ...
                for _ in range(50):
                    ...
                    while True:
                        ...
                        if terminal and collisions > 5 and not done:
                            ...
                            failure_count_list.append([regular_time, failure_by_diffusion])
                            break
                        if terminal or done:
                            break
                    ...
                    if failure_flag:
                        ...
                        failure_cluster_count_list.append([regular_time, len(set(diffusion_failure_clusters))])
    ...
    with open('./results/plot_data.pickle', 'wb') as f:
        pickle.dump((len(information_list), len(novelty_dict.keys()), failure_count_list, failure_cluster_count_list), f)

    regular_time, _, test_cases = zip(*diffusion_failure_count)
    with open("GM"+datetime.now().strftime("%m_%d_%Y_%H_%M_%S"), "wb") as f:
        pickle.dump((all_seeds, test_cases, np.array(regular_time)*3600), f)
    print('finish')
