...
def parse_args():
    ...
    parser.add_argument("--intrinsic", type=int, default=200, )
    parser.add_argument("--entropy", type=int, default=200)
...
def sample_seeds(seed_num=100):
    ...
    with U.single_threaded_session():
        ...
        while seed_index < seed_num:
            ...
            if done or terminal:
                intrinsic_reward = fuzzer.train_rnd(state_seqs)
                fuzzer.further_mutation(copy.deepcopy(init_state), episode_rewards, None, intrinsic_reward, np.asarray(state_seqs[-1]), copy.deepcopy(init_state))
                obs_n = env.reset()
                state_seqs = []
                init_state = get_init_state(env)
                state_seqs.append(get_observe(env))
        ...
        for i in range(len(fuzzer.corpus)):
            ...
            while True:
                ...
                if done or terminal:
                    fuzzer.entropy[i] =  np.linalg.norm(fuzzer.final_state[i] - np.asarray(get_observe(env_uncertainty)))
                    break
    return fuzzer, trainers

def test(arglist, result_folder):
    np.random.seed(arglist.seed)
    intrinsic_threshold = arglist.intrinsic
    entropy_threshold = arglist.entropy
    ...
    with U.single_threaded_session():
        ...
        state_seqs.append(get_observe(env))
        collisions = 0
        init_state = copy.deepcopy(get_init_state(env))
        episode_step = 0
        all_seeds = []
        result_time = []
        ...
        while current_fuzz_time - start_fuzz_time < 3600 * 1 and len(fuzzer.corpus) > 0:
            ...
            if terminal and collisions > 5 and not done:
                ...
                result_time.append(time.time() - start_fuzz_time)
            elif done or terminal:
                current_fuzz_time = time.time()
                intrinsic_reward = fuzzer.train_rnd(state_seqs)
                if episode_rewards < fuzzer.current_reward or intrinsic_reward > intrinsic_threshold or entropy > entropy_threshold:
                    entropy = np.linalg.norm(fuzzer.current_final_state - np.asarray(get_observe(env)))
                    fuzzer.further_mutation(copy.deepcopy(init_state), episode_rewards, entropy, intrinsic_reward, np.asarray(state_seqs[-1]), fuzzer.current_original)
            if done or terminal:
                current_fuzz_time = time.time()
                current_pos = fuzzer.get_pose()
                new_pos = fuzzer.mutate(current_pos)
                ...
                while agent_flag or landmark_flag:
                    ...
                    if mutation_count > 10 and (agent_flag or landmark_flag):
                        ...
                        mutation_count = 0
                        current_pos = fuzzer.get_pose()
                all_seeds.append(tuple(new_pos))
                state_seqs = []
                init_state = copy.deepcopy(get_init_state(env))
                state_seqs.append(get_observe(env))
                episode_step = 0
    ...
    with open('Cure'+result_folder, 'wb') as handle:
        pickle.dump((all_seeds, fuzzer.result, result_time), handle, protocol=pickle.HIGHEST_PROTOCOL)

if __name__ == '__main__':
    arglist = parse_args()
    f = open('./results'+result_folder+'/cure_fuzzing.txt', 'w', buffering=1)
    sys.stdout = f
    test(arglist, result_folder)
