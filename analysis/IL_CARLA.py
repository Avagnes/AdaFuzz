...
class IntData:
    def __init__(self, algo) -> None:
        ...
        file_paths = file_paths['IL_CARLA'][algo]
        self.dis_map_list = []
        for file_path in file_paths:
            with open(file_path, 'rb') as f:
                all_seeds, result, result_time = pickle.load(f)
                standardized_states = [np.array([float(s_k) for s_k in s]) for s in all_seeds]
                ...
                for i in tqdm.tqdm(range(LEN)):
                    ...
                    for j in range(i+1,LEN):
                        ...
                        max_distance = np.sqrt(40804*4+72900+40804*(len(standardized_states[i])-6))
...
for algo in ['MDPFuzz', 'G-Model', 'CureFuzz', 'AdaFuzz']:
    ...
    for eta in np.linspace(0, 0.05, 40):
        ans = searcher(eta)
        data.append((eta, ans))
        with open('./total/IL_CARLA/'+algo+'.pickle', 'wb') as f:
            pickle.dump(data, f)
