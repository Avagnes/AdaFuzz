...
def run(model_path, port, suite, big_cam, seed, autopilot, resume, max_run=10, show=False, replay=False, em_guide=True, args = None):
    ...
    for suite_name in get_suites(suite):
        ...
        with make_suite(suite_name, port=port, big_cam=big_cam) as env:
            ...
            elif args.algo == 'adafuzz':
                print('ablate_adjust:', args.ablate_adjust, ', adlate_update:', args.ablate_update)
                if args.ablate_update and not args.ablate_adjust:
                    from benchmark.run_ada_ablate_update import run_benchmark
                elif not args.ablate_update and args.ablate_adjust:
                    from benchmark.run_ada_ablate_adjust import run_benchmark
                elif not args.ablate_update and not args.ablate_adjust:
                    from benchmark.run_ada import run_benchmark
                run_benchmark(agent_maker, env, benchmark_dir, seed, resume, result_folder, max_run=max_run, show=show, replay=replay, args=args)
            elif args.algo == 'curefuzz':
                from benchmark.run_cure import run_benchmark
                run_benchmark(agent_maker, env, benchmark_dir, seed, resume, result_folder, max_run=max_run, show=show, replay=replay)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--algo', type=str, default='mdpfuzz', choices=['mdpfuzz', 'adafuzz', 'curefuzz'])
    ...
    parser.add_argument("--hour", help="test time", default=8, type=int)
    ...
    parser.add_argument("--ablate_update", default=False, action='store_true')
    parser.add_argument("--ablate_adjust", default=False, action='store_true')
