...
from fuzz.ada_fuzz_ablate_update import AdaFuzz, SeedSpace
...
def run_single(env, weather, start, target, agent_maker, seed, log, replay=False, guide=True):
    ...
    if replay == True:
        ...
        with open('carla0', 'rb') as f:
            normal_cases = pickle.load(f)

        def add_family():
            nonlocal all_seeds, result, result_time, fuzzer, env, agent, pbar, start_fuzz_time, normal_cases
        ...
        while time.time() - start_fuzz_time < 3600 * 2:
            ...
            pbar.update(1)
        with open("ablate_update", "wb") as f:
